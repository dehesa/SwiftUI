    ...
    var body: some View {
        VStack {
            GeometryReader { geo in
                ...
                .background(Color.white.shadow(color: .black, radius: 5))
                .onTapGesture { withAnimation { self.showingPunchline.toggle() } }
            ...
            .onEnded { value in
                if self.dragAmount.height < -200 {
                    withAnimation {
                        ...
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) { self.moc.delete(self.joke) }
                    }
                } else {
