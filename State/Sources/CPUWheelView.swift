...
struct CPUWheelView: View {
    ...
    var body: some View {
        ...
        return Circle()
            ...
            .onReceive(timer) { _ in withAnimation { self.cpu = Int(CPUWheelView.cpuUsage()) }
        }
    }
    ...
    struct CPUClip: Shape {
        ...
        func path(in rect: CGRect) -> Path {
            ...
            path.move(to: c)
            path.addArc(center: c, radius: rect.width/2.0, startAngle: Angle(degrees: 0), endAngle: Angle(degrees: (pct/100.0) * 360), clockwise: false)
            path.closeSubpath()
            return path
        }
    }
